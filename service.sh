...
# Sister-project port map (kept in sync across all testbot service.sh scripts).
# A start/restart aborts if BACKEND_PORT is held by a process we don't own.
SISTER_PORTS_DOC="\
  3001 = romero-tech-solutions
  3002 = funder-finder
  3003 = worship-setlist
  3004 = tampa-re-investor (Coastly)
  8100 = ZenithGrid (this project)"

# True if our backend port is held by a process that's NOT our systemd unit.
# Echoes the foreign PID on stdout when so.
...
start_backend() {
    ...
    if [ "$OS_TYPE" = "Darwin" ]; then
        ...
        if foreign_pid=$(foreign_holds_backend_port); then
            echo -e "${RED}Port ${BACKEND_PORT} is held by foreign PID ${foreign_pid}.${NC}"
            echo -e "${YELLOW}Sister-project port map:${NC}"
            echo -e "$SISTER_PORTS_DOC"
            echo -e "${YELLOW}Refusing to start. Investigate: sudo ss -tlnp | grep :${BACKEND_PORT}${NC}"
            return 1
        fi
