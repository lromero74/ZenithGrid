"""Test buy ETH and check what Coinbase returns about the fill"""
import asyncio
...
async def test_buy():
    ...
    print("=" * 60)
    print("Buying 0.00032 BTC worth of ETH (market order)")
    print("=" * 60)
    ...
    order = await coinbase.create_market_order(
        product_id="ETH-BTC",
        side="BUY",
        funds="0.00032"  # 0.00032 BTC worth
    )
    ...
    if order.get("success", False):
        ...
        print(f"Filled Size (ETH): {filled_size}")
