...
async def get_grok_analysis(
    ...
    # Initialize Grok client - check database first, then .env
    api_key = get_api_key_for_provider_sync(user_id, "grok")
    if not api_key:
        logger.error("Grok API key not configured (checked database and .env)")
        return {
            "signal_type": "hold",
            "confidence": 0,
