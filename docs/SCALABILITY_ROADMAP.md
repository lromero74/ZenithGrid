...
- [ ] **Finish trader session-scope refactor: limit/safety order monitors.** v3.13.19 closed the MultiBotMonitor scheduler's parent DB session before launching concurrent bot work, which removes one long-held parent transaction. Live `pg_stat_activity` still showed `idle in transaction` samples from lazy-loaded bot products, blacklist rows, and trade collections, and static review found the next larger target: `check_all_pending_limit_orders()` / `check_all_pending_safety_orders()` keep one DB session open while calling exchange `get_order`, ticker, edit/cancel, replacement order creation, WebSocket/event-bus notification, and post-fill operations. Recommended next no-infrastructure step: snapshot pending order/position IDs per account in a short read session, close it, do exchange polling outside any DB transaction, then reopen a fresh short write session per fill/status update. Treat this as a focused trading-safety release with tests for partial fills, cancel+replace, paper orders, and safety-order adds.
