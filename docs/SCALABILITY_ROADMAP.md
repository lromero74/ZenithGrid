...
- [x] **Finish parent transaction split for limit/safety order monitors.** v3.13.19 closed the MultiBotMonitor scheduler's parent DB session before launching concurrent bot work. v3.13.20 moved `check_all_pending_limit_orders()` and `check_all_pending_safety_orders()` onto a scoped read-poll-write path: snapshot pending order/position IDs per account in a short read session, close it, poll exchange order status outside any DB transaction, then reopen a fresh short write session per fill/status update. Remaining nuance: rare bid-fallback cancel/replace still performs its exchange edit/cancel/create sequence from inside the per-order write helper; if `idle in transaction` persists specifically around aged limit-close repricing, split that sub-flow next with focused cancel+replace tests.
- [ ] **Split remaining trader read sessions that still show as idle in transaction.** After v3.13.21 deploy, PostgreSQL was healthy overall (26/100 connections, no pool errors), but 30-second samples still showed 3-4 `idle in transaction` rows. The longest recurring query was `bot_rebalancer_groups`, with shorter `blacklisted_coins`, `positions`, and `trades` reads. Next no-infrastructure cleanup: apply the same scoped read/compute/write pattern to rebalancer-group/blacklist/position refresh paths, starting with focused tests around `bot_rebalancer_groups` so background reads do not sit in open transactions while trader work continues.
