...
- [~] **Manual-mode DCA base-order back-calc breaks with volume scaling** — DEFERRED (real-money sizing, needs care). Only bites non-`auto_calculate` + `percentage_of_base` + `volume_scale≠1` bots; the `fixed`/`fixed_btc` SO types ignore the back-calc entirely. A correct fix needs the first entry trade's size (ground truth) or a stored `base_order_size`, plus fixing the cascade count passed at `:952`. Deserves its own focused change + parity test, not a batch edit. `indicator_based.py`
