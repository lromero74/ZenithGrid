...
import { User, Lock, CheckCircle, AlertCircle, Database, Trash2, Shield, Copy, CheckSquare, Monitor, MapPin, Clock, X as XIcon, Mail, Smartphone, Palette, RefreshCw } from 'lucide-react'
...
function AdminDisplayNameField() {
  const [editing, setEditing] = useState(false)
  const [value, setValue] = useState('')
  const [saved, setSaved] = useState<string | null>(null)
  const [saving, setSaving] = useState(false)

  // Load current admin display name
  useEffect(() => {
...
export default function Settings() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Active Sessions Section — settings:write only */}
      {canWriteSettings && <ActiveSessions />}
