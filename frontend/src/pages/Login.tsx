...
export default function Login() {
  ...
  const disposableDomainsRef = useRef<Set<string> | null>(null)
  ...
  // Load disposable domain list from backend once when signup modal opens
  useEffect(() => {
    if (!showSignup || disposableDomainsRef.current) return
    fetch('/api/auth/disposable-domains')
      .then(r => r.ok ? r.json() : null)
      .then(data => {
        if (data?.domains) disposableDomainsRef.current = new Set(data.domains)
      })
      .catch(() => { /* non-critical — fail silently */ })
  }, [showSignup])

  const looksAutoGenerated = (localPart: string): boolean => {
    // Mirror of backend looks_machine_generated heuristic:
    // all alphanumeric, length 8-20, >= 5 digits, <= 1 vowel
    const lp = localPart.toLowerCase()
    if (!/^[a-z0-9]+$/.test(lp)) return false
    if (lp.length < 8 || lp.length > 20) return false
  ...
  const checkDisposableEmail = (email: string) => {
    ...
    const isDomainDisposable = disposableDomainsRef.current?.has(domain) ?? false
    const isLocalSuspicious = looksAutoGenerated(local)
    const suspicious = isDomainDisposable || isLocalSuspicious
    setEmailIsDisposable(suspicious)
    if (!suspicious) setDisposableAcknowledged(false)
  }
          ...
          <div className="w-full max-w-lg bg-slate-800 rounded-lg shadow-2xl border border-slate-700 max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6">
              ...
              <form onSubmit={handleSignup} className="space-y-5">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      type="email"
                      value={signupEmail}
                      onChange={(e) => { setSignupEmail(e.target.value); setEmailIsDisposable(false); setDisposableAcknowledged(false) }}
                      onBlur={(e) => checkDisposableEmail(e.target.value)}
                      required
                      autoComplete="email"
                      placeholder="you@example.com"
                      className={`w-full pl-10 pr-4 py-3 bg-slate-700 border rounded-lg text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:border-transparent ${emailIsDisposable ? 'border-yellow-500 focus:ring-yellow-500' : 'border-slate-600 focus:ring-blue-500'}`}
                    />
                  </div>
                </div>
                ...
                {emailIsDisposable && (
                  <div className="p-4 bg-yellow-500/10 border border-yellow-500/50 rounded-lg space-y-3">
                    <div className="flex items-start gap-3">
                      <TriangleAlert className="w-5 h-5 text-yellow-400 flex-shrink-0 mt-0.5" />
                      <div className="text-sm text-yellow-200 space-y-1">
                        <p className="font-semibold text-yellow-300">This email address looks like a throwaway</p>
                        <p>Accounts registered with disposable or auto-generated email addresses are likely to be banned, and your IP address may be blocked. If you have a real email address, please use that instead.</p>
                        <p>If you still want to proceed, check the box below.</p>
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() => setDisposableAcknowledged(!disposableAcknowledged)}
                      className="flex items-center gap-2 text-sm text-yellow-200"
                    >
                      {disposableAcknowledged
                        ? <CheckSquare className="w-4 h-4 text-yellow-400 flex-shrink-0" />
                        : <Square className="w-4 h-4 text-yellow-500/70 flex-shrink-0" />}
                ...
                <button
                  type="submit"
                  disabled={isSigningUp || !signupEmail || !isPasswordValid(signupPassword) || signupPassword !== signupConfirmPassword || !agreedToTerms || (emailIsDisposable && !disposableAcknowledged)}
                  className="w-full py-3 px-4 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-600 disabled:cursor-not-allowed text-white font-medium rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800"
                >
                  {isSigningUp ? (
                    <span className="flex items-center justify-center space-x-2">
