...
export const OverallStatsPanel = ({ stats, completedStats, realizedPnL, balances, onRefreshBalances }: OverallStatsPanelProps) => {
  ...
  const [selectedToDate, setSelectedToDate] = useState<'wtd' | 'mtd' | 'qtd' | 'ytd'>(() => {
    try { return (localStorage.getItem('zenith-stats-to-date') as 'wtd' | 'mtd' | 'qtd' | 'ytd' | null) || 'ytd' } catch { return 'ytd' }
  })
  const [selectedCumulative, setSelectedCumulative] = useState<'alltime' | 'net'>(() => {
    try { return (localStorage.getItem('zenith-stats-cumulative') as 'alltime' | 'net' | null) || 'alltime' } catch { return 'alltime' }
  })
    ...
    <div className="bg-slate-800 rounded-lg border border-slate-700 p-3 sm:p-6 mb-4">
      <div className="grid grid-cols-1 md:grid-cols-[2fr_1.2fr_2fr] gap-6">
        ...
        <div>
          ...
          <div className="space-y-2">
            <div className="flex items-baseline text-slate-400 text-xs gap-1 sm:gap-3">
            ...
            <div className="flex items-baseline gap-1 sm:gap-3">
              ...
              <span className="text-blue-400 flex-1 min-w-0 text-right font-mono text-[10px] sm:text-xs">
                {(stats.totalBudgetByQuote.BTC || 0).toFixed(6)}
              </span>
              <span className="text-amber-400 flex-1 min-w-0 text-right font-mono text-[10px] sm:text-xs">
                {balances ? balances.reserved_in_positions.BTC.toFixed(6) : '...'}
              </span>
              <span className="hidden sm:flex sm:flex-1 sm:min-w-0 text-purple-400 text-right font-mono text-xs justify-end">
                {balances ? balances.reserved_in_pending_orders.BTC.toFixed(6) : '...'}
              </span>
              <span className="text-green-400 flex-1 min-w-0 text-right font-mono text-[10px] sm:text-xs font-semibold">
