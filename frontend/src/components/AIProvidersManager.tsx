...
const PROVIDER_CONFIG: Record<string, { name: string; description: string; color: string }> = {
  claude: {
  ...
  openai: {
    name: 'OpenAI',
    description: 'GPT models for trading decisions',
    color: 'text-cyan-400',
  },
}

const PROVIDERS = ['claude', 'gemini', 'grok', 'groq', 'openai']
...
export function AIProvidersManager() {
  ...
  const getProviderInfo = (provider: string): AIProviderStatus | undefined => {
    return providerStatus.find((p) => p.provider === provider)
  }
  ...
  const handleDeleteKey = async (provider: string) => {
    if (!confirm(`Are you sure you want to delete your ${PROVIDER_CONFIG[provider]?.name || provider} API key?`)) {
      return
    }
      ...
      {saveSuccess && (
        <div className="mb-4 p-4 bg-green-500/10 border border-green-500/50 rounded-lg flex items-center space-x-3">
          ...
          <p className="text-green-400 text-sm">
            API key saved successfully for {PROVIDER_CONFIG[saveSuccess]?.name || saveSuccess}!
          </p>
        </div>
      )}
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <Key className={`w-5 h-5 ${config?.color || 'text-slate-400'}`} />
                  <div>
                    <h4 className="font-medium text-white">{config?.name || provider}</h4>
                  ...
                  <div className="flex items-center space-x-3">
                    <div className="flex-1 relative">
                      <input
                        ...
                        placeholder={`Enter your ${config?.name || provider} API key`}
