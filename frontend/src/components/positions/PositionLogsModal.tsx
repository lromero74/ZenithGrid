...
import { botsApi, positionsApi, type BotLogEntry, type IndicatorConditionDetail } from '../../services/api'
import { Brain, TrendingUp, TrendingDown, CircleDot, Clock, Target, X, BarChart3, CheckCircle, XCircle, Coins } from 'lucide-react'
...
type TimelineItem =
  | { kind: 'log'; ts: number; log: BotLogEntry }
  | { kind: 'order'; ts: number; order: OrderEvent }

function PositionLogsModal({ botId, productId, positionId, positionOpenedAt, isOpen, onClose }: PositionLogsModalProps) {
  ...
  // The actual order fills for this position (base order, safety orders, close).
  // These are the mechanical actions a price-deviation DCA bot takes that never
  // produce an indicator "decision" — surfaced here so the timeline reflects what
  // really happened, retroactively, for any bot type.
  const { data: trades = [] } = useQuery({
    queryKey: ['position-trades', positionId],
    queryFn: () => positionsApi.getTrades(positionId),
    enabled: isOpen && positionId > 0,
    refetchInterval: isOpen ? 10000 : false,
  })

  const orderEvents = useMemo(() => buildOrderEvents(trades), [trades])
  ...
  // Merge order fills with decision logs into one chronological timeline (newest
  // first). Order events only show under the "All Logs" filter — the other
  // filters are about AI/indicator decision types.
  const timelineItems: TimelineItem[] = filteredLogs.map((log) => ({
    kind: 'log' as const, ts: new Date(log.timestamp).getTime(), log,
  }))
  if (filterDecision === 'all') {
    for (const order of orderEvents) {
      timelineItems.push({ kind: 'order' as const, ts: new Date(order.timestamp).getTime(), order })
    }
  }
  const timeline: TimelineItem[] = timelineItems.sort((a, b) => b.ts - a.ts)

  const quoteSymbol = productId.includes('-') ? productId.split('-')[1] : ''
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {isLoading ? (
            ...
            <div className="text-center text-slate-400 py-8">
              {filterDecision === 'all'
                ? 'No decision logs or orders for this position yet.'
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <Coins className={`w-5 h-5 ${isBuy ? 'text-green-400' : 'text-red-400'}`} />
                        <div>
                          <div className="flex items-center space-x-2">
                            <span className="px-1.5 py-0.5 bg-amber-600/20 border border-amber-600/50 text-amber-300 rounded text-xs font-medium">
                              ORDER
                            </span>
                            <span className="text-sm font-semibold text-slate-200">{order.label}</span>
                          </div>
                      ...
                      <div className="text-right text-xs text-slate-400">
                        <div className="text-slate-200">
                          {order.baseAmount.toFixed(8)} {productId.split('-')[0]}
                        </div>
                        <div>@ {order.price.toFixed(8)} {quoteSymbol}</div>
                      </div>
                    </div>
                  </div>
                )
