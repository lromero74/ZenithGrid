...
- **Savings Targets**: Track lump-sum savings goals alongside regular expenses — vacation fund, car, emergency fund. Uses PMT formula to calculate required monthly contribution based on target amount, target date, current balance, and assumed annual growth rate. Multiple targets per goal, each with its own progress bar and on-track status. Recurring targets use a self-sustaining hold model — the accumulation target includes a seed that compounds back to cover the next cycle's withdrawal (including tax) without further contributions. The report breakdown shows `Spend · Tax · Hold → accumulate` for full transparency. Targets show "Ready" badge when the balance is sufficient to make the full withdrawal right now. Savings targets are rendered in PDF reports.
