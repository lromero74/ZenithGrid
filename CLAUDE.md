...
- **Process-role fencing:** `PROCESS_ROLE=combined` is the safe default and current production mode. `web` skips every trading monitor and scheduler; `trader` runs them. Both `combined` and `trader` must acquire and continuously renew the token-checked Redis key `zenith:trading-leader`; contention aborts startup and lease loss exits fail-closed. Never bypass, delete, rename, or manually forge this key while a trader is running. A future split deployment must verify `/api/health.process_role` for each process before nginx is switched.
